...
if ( !($_SERVER["HTTPS"] )) {
	header( "Location: " . $site->admin_dir );
	exit;
}
...
$title   = "Transaction Solutions Administration Portal Login";
...
if ( !isset($_SESSION["user_id"])) {
	...
	if ( isset($_POST["user_id"]) && isset($_POST["password"]) ) {
		...
		$auth  = "SELECT * FROM admin WHERE id ='";
		...
		if ( $my_user = $db->get_row($auth) ) {
			...
			$_SESSION["email"]     = $my_user->email;
			$_SESSION["adminRank"] = $my_user->adminRank;
